...
export const DOMAIN_CONFIGS: DomainConfig[] = [
  ...
  },
  {
    ...
    extractCore: (d) => ({
      ...
      content: str(d["谈话内容"]),
      talkDate: str(d["谈话日期"]),
      nextDate: str(d["下次回访"] ?? d["下次谈话日期"] ?? ""),
      cycleDays: num(d["回访周期"] ?? d["周期天数"]),
    }),
    numericColumns: ["回访周期"],
  },
  {
    featureId: "crisis-records",
    table: crisisRecords,
    extractCore: (d) => ({
      studentNo: str(d["学号"]),
      studentName: str(d["姓名"]),
      className: classNameOf(d),
      discoverWay: str(d["发现方式"]),
      crisisLevel: str(d["危机等级"]),
      reason: str(d["危机原因"] ?? d["危机情况"] ?? ""),
      intervention: str(d["干预措施"] ?? d["干预预案"] ?? ""),
      followup: str(d["跟进记录"] ?? ""),
      responsible: str(d["负责老师"] ?? ""),
      status: str(d["危机状态"] ?? "跟踪中"),
    }),
    numericColumns: [],
  },
  {
    featureId: "help-records",
    table: helpRecords,
    extractCore: (d) => ({
      studentNo: str(d["学号"]),
      studentName: str(d["姓名"]),
      className: classNameOf(d),
      courseName: str(d["挂科课程"] ?? d["课程名称"] ?? ""),
      score: num(d["成绩"] ?? d["课程成绩"]),
      measure: str(d["帮扶措施"] ?? d["帮扶计划"] ?? ""),
      cycle: str(d["帮扶周期"] ?? ""),
      effect: str(d["帮扶效果"] ?? ""),
      status: str(d["帮扶状态"] ?? "帮扶中"),
    }),
    numericColumns: ["成绩"],
  },
];
